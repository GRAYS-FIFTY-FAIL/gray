...
def words_check(request):
    ...
    # 워드클라우드
    taglist = pytagcloud.make_tags(nouns, maxsize=40)
...
def senti_check(request):
    ...
    for data in words.keys():
        ...
        a, b = senti_dict.Singleton().data_list(wordname)
        pos.append((wordname, a, b, words.get(data)))
        if(a == 'POS'):
            pos_p = pos_p + b * words.get(data)
            pos_c = pos_c + words.get(data)
            total = total + b * words.get(data)
        elif(a == 'NEG'):
            neg_p = neg_p + b * words.get(data)
            neg_c = neg_c + words.get(data)
            total = total - (b * words.get(data))
    ...
    pos.sort(key = lambda element : element[3],reverse=True)
    context = {
        'pos':pos,
    }
    return HttpResponse(json.dumps(context), content_type='application/json')
